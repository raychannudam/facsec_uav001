  ...
  <div class="relative">
    ...
    <div class="absolute bg-black/70 right-0 m-10 flex flex-col z-40 space-y-3 p-3 text-sm items-start rounded-md">
      ...
      <div class="w-full grid grid-cols-2  gap-3">
        ...
        <div>
          <p class="text-xs">All <span class="block text-xl font-bold"><span v-if="allStations.length < 10">0</span>{{
            allStations.length }}</span></p>
        </div>
        <div>
          <p class="text-xs">Active<span class="block text-xl font-bold"><span v-if="allStations.length < 10">0</span>{{
            allStations.length }}</span></p>
        </div>
        <div class="flex items-center space-x-1 text-sm col-span-2 border-b">
          <span class="material-symbols-outlined text-sm">
