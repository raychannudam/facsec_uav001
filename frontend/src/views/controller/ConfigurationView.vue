<template>
    <div class="flex flex-col space-y-3 h-full">
        ...
        <div class="flex flex-col w-full space-y-2">
            ...
            <!-- Actions (Auto-Wrap & Shrink) -->
            <div class="flex flex-wrap gap-2">

                <!-- New -->
                <button @click="createNewProfile" class="flex items-center gap-1 px-3 py-1 text-xs font-medium text-white bg-green-700 hover:bg-green-800
                ...
                <!-- Edit -->
                <button v-if="!isEditing && selectedProfile" @click="isEditing = true" class="flex items-center gap-1 px-3 py-1 text-xs font-medium text-white bg-yellow-600 hover:bg-yellow-700
                ...
                <!-- Save -->
                <button v-if="selectedProfile" @click="saveProfile" class="flex items-center gap-1 px-3 py-1 text-xs font-medium text-white bg-blue-700 hover:bg-blue-800
                ...
                <!-- Delete -->
                <button v-if="selectedProfile && !isEditing" @click="deleteProfile" class="flex items-center gap-1 px-3 py-1 text-xs font-medium text-white bg-red-700 hover:bg-red-800
                   rounded-lg focus:ring-2 focus:ring-red-300">
        ...
        <div class="flex flex-row space-x-3 items-center justify-start">
            <p class="text-sm font-bold">Selected Profile</p>
                ...
                <ul class="h-48 px-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200"
                    ...
                    <li v-for="profile in filteredProfiles" :key="profile.id">
                        <div class="flex items-center ps-2 rounded-sm hover:bg-gray-100 dark:hover:bg-gray-600">
                            <input :id="'profile-' + profile.id" type="radio" :value="profile" v-model="selectedProfile"
        ...
        <div v-if="selectedProfile" class="flex flex-col space-y-3 h-[60vh] overflow-scroll relative"
            :class="{ 'pointer-events-none opacity-70': !isEditing }">

            <!-- Profile Name -->
            <div class="flex flex-row space-x-3 items-center justify-start" v-if="isEditing">
            ...
            <div class="flex flex-col space-y-3" v-if="availableMqttTopics.length > 0">
                <TopicAssignComponent v-for="mqttTopic in config.mqttTopics" :key="mqttTopic.id" :type="mqttTopic.type"
                    :id="mqttTopic.id" :name="mqttTopic.name" dropDownDesc="Select a topic" :data="availableMqttTopics"
...
export default {
    ...
    methods: {
        ...
        async onProfileSelect() {
            ...
            // Reinitialize Flowbite dropdowns after profile change
            await this.$nextTick();
            initFlowbite();
        },
        ...
        loadProfileConfig() {
            ...
            this.config = {
                ...
                streamingUrls: this.selectedProfile.config?.streamingUrls || this.getDefaultConfig().streamingUrls,
                mqttTopics: this.selectedProfile.config?.mqttTopics?.map(topic => {
                    const defaultTopic = this.getDefaultConfig().mqttTopics.find(t => t.id === topic.id);
                    return defaultTopic ? { ...defaultTopic, ...topic } : topic;
                }) || this.getDefaultConfig().mqttTopics
            };

            if (Object.keys(this.config.selectedDrone).length > 0) {
                this.selectedDrone = this.config.selectedDrone;
                this.loadDroneResources();
            } else {
                // Clear drone selection if profile has no drone
        ...
        async createNewProfile() {
            ...
            if (!profileName || profileName.trim() === "") {
                this.appStore.displayRightToast("fail", "Profile name cannot be empty");
                return;
            }
            ...
            if (res.status === "success") {
                await this.loadProfiles();
                this.selectedProfile = this.allProfiles.find(p => p.name === profileName.trim());
                this.loadProfileConfig();
                this.isEditing = true;
                this.$emit("onUpdate");
            }
        ...
        async saveProfile() {
            ...
            this.appStore.displayPageLoading(true);
            let res = await this.controllerStore.updateController(this.selectedProfile.id, data);
            this.appStore.displayPageLoading(false);

            if (res.status === "success") {
                this.isEditing = false;
                await this.loadProfiles();
                this.selectedProfile = this.allProfiles.find(p => p.id === this.selectedProfile.id);
                this.loadProfileConfig();
                this.$emit("onUpdate");
            }
        },
        ...
        async deleteProfile() {
            ...
            if (!confirm(`Are you sure you want to delete profile "${this.selectedProfile.name}"?`)) {
                return;
            }
