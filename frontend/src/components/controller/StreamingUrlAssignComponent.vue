<template>
  <div class="flex flex-row space-x-3 items-center justify-start relative">
    ...
    <button :id="id + 'Button'" :data-dropdown-toggle="id" data-dropdown-placement="bottom"
      ...
      type="button">
      <p class="flex-1 start" v-if="!selectedUrl || Object.keys(selectedUrl).length === 0">{{ dropDownDesc }}</p>
      <p class="flex-1 start" v-else>{{ selectedUrl.name }}</p>
      ...
      <ul class="h-48 px-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200"
        ...
        <li class="w-full">
          <div class="flex items-center rounded-sm hover:bg-gray-100 dark:hover:bg-gray-600">
            <input :id="'checkbox-item-' + id + '- default'" type="radio" :value="{}" v-model="selectedUrl"
              class="hidden">
            <label :for="'checkbox-item-' + id + '- default'"
              class=" inline-flex items-center justify-center space-x-3 w-full py-2 text-sm font-medium text-white text-center cursor-pointer rounded-md bg-blue-500"><span
        ...
        <li v-for="item in data">
          <div class="flex items-center ps-2 rounded-sm hover:bg-gray-100 dark:hover:bg-gray-600">
            <input :id="'checkbox-item-' + id + '-' + item.name" type="radio" :value="item" v-model="selectedUrl"
              class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
            <label :for="'checkbox-item-' + id + '-' + item.name"
              class="w-full py-2 ms-2 text-sm font-medium text-gray-900 rounded-sm dark:text-gray-300">{{ item.name
              }}</label>
          </div>
        </li>
      </ul>
