    ...
    <Teleport to="body">
        <div v-if="isOpen" class="fixed inset-0 z-50 overflow-y-auto" style="background-color: rgba(0, 0, 0, 0.5);">
            <div class="min-h-screen px-4 flex items-center justify-center">
                <div
                    class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[85vh] overflow-hidden flex flex-col">
                    <div
...
watch(() => props.drone, (newDrone) => {
    if (newDrone) {
        formData.value = {
            name: newDrone.name || '',
            type: newDrone.type || '',
            mqtt_client_id: newDrone.mqtt_client?.id || newDrone.mqtt_client_id || '',
            streaming_client_id: newDrone.streaming_client?.id || newDrone.streaming_client_id || '',
