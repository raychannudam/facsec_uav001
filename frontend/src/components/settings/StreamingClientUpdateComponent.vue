<template>
    <PopupModalComponent :id="'streaming-popup-modal-detail' + data.id" @onSave="updateClient" @onClose="updateClientModalClosed">
        <template v-slot:header>
            <div>
                <p class="text-xs">Editing Streaming client name <span class="block text-xl font-bold">{{ data.name }}</span>
                </p>
            </div>
        </template>
        <template v-slot:body>
            <div class="flex flex-col space-y-3">
                ...
                <div class="relative">
                    <JsonEditorVue v-model="editingConfig" mode="text"></JsonEditorVue>
                </div>
            </div>
        </template>
    </PopupModalComponent>
...
export default {
    ...
    data() {
        return {
            ...
            editingConfig: this.data.config
        }
    },
    methods:{
        async updateClient(id){
            ...
            let data = {
                ...
                config: typeof this.editingConfig === 'string' ? JSON.parse(this.editingConfig) : JSON.parse(JSON.stringify(this.editingConfig))
            }
            let res = await this.settingStore.updateStreamingClient(this.data.id, data);
